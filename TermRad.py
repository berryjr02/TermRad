...
class TermRad(App):
    ...
    def on_mount(self) -> None:
        #hardcoded to flint for the sake of testing, this needs to be changed
        forecast_data = get_numerical_forecast(43.0125, -83.6875)
            
        #dynamically creates a widget for each "item" of forecast data that we retrieve
        forecast_widgets = [
            ForecastWidget(period, id=f"forecast_{i}", classes="forecast_item")
            for i, period in enumerate(forecast_data)
        ]
            

        container = self.query_one("#forecast_container", ScrollableContainer)
        #container = self.query_one("#forecast_container", Static)
